...
export function ExamsSection({ dict, locale }: ExamsSectionProps) {
    ...
    return (
        <section id="exams" className="relative py-24 xl:py-32">
                ...
                <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
                    ...
                    >
                        ...
                        <div className="flex md:justify-end">
                            ...
                            >
                                {dict.exams.viewAll}
                                <div className="flex h-8 w-8 items-center justify-center rounded-full border border-teal-100/80 bg-white/80 backdrop-blur-sm transition-transform group-hover:translate-x-1">
                                    <ArrowRight size={16} />
                                </div>
                            </Link>
                        </div>
                    ...
                    {examDetails.map((exam, index) => {
                        ...
                        return (
                            <motion.article
                                ...
                                className={`group relative overflow-hidden rounded-[40px] border ${exam.border} bg-white/72 p-8 backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:bg-white/90 hover:shadow-[0_40px_80px_-20px_rgba(16,153,136,0.15)]`}
                            >
                                <div
                                    className={`absolute -right-10 -top-10 h-32 w-32 rounded-full bg-gradient-to-br ${exam.gradient} opacity-[0.08] blur-2xl transition-transform duration-700 group-hover:scale-150`}
                                />
                                ...
                                <div className="relative">
                                    ...
                                    <ul className="mt-8 space-y-3">
                                        {examData.features.map((feature: string, fIndex: number) => (
                                            ...
                                            >
                                                <div
                                                    className={`h-1.5 w-1.5 rounded-full bg-gradient-to-r ${exam.gradient}`}
                                                />
                                                {feature}
                                            </li>
                                        ))}
                                    ...
                                    <div className="mt-10">
                                        <Link
                                            ...
                                            className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-teal-100/80 bg-white/85 px-6 py-4 text-sm font-bold text-teal-900 backdrop-blur-sm shadow-sm transition-all hover:bg-teal-950 hover:text-white hover:shadow-md"
                                        >
