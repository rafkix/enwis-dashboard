...
import { ArrowRight, BookOpenCheck, CheckCircle2,
    Headphones, Mic, PenSquare, Target, Star, AlertCircle,
} from "lucide-react";
...
import { CEFRPlatform } from "@/components/ielts/IELTSPlatform";
...
export default async function CEFRPage({
    ...
    const page = dict.cefrPage;

    return (
        <main className="min-h-screen bg-white font-sans text-slate-900 selection:bg-red-600 selection:text-white">
            <CEFRHero locale={locale as Locale} page={page.hero} />
            <CEFRIntro page={page.intro} />
            <CEFRFormat page={page.format} />
            <CEFRScoring page={page.scoring} />
            <CEFRChallenges page={page.challenges} />
            <CEFRPlatform locale={locale as Locale} page={page.platform} />
...
function CEFRHero({ locale, page }: { locale: Locale; page: any }) {
    return (
...
function CEFRIntro({ page }: { page: any }) {
    return (
...
function CEFRFormat({ page }: { page: any }) {
...
function CEFRScoring({ page }: { page: any }) {
    return (
                    ...
                    <div className="flex flex-col gap-4">
                        {["C1", "B2", "B1"].map((num) => (
                            <div key={num} className="bg-white p-6 rounded-3xl flex items-center justify-between shadow-2xl transition-transform hover:scale-105">
                                ...
                                <span className="text-4xl font-black italic text-red-600">{num}</span>
                            </div>
                        ))}
                    </div>
                </div>
...
function CEFRChallenges({ page }: { page: any }) {
    return (
        <section className="py-24 bg-white">
